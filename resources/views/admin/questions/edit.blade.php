...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <!-- Form -->
    <form method="POST" action="{{ route('admin.exams.questions.update', [$exam, $question]) }}" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="ios-card p-6 space-y-6">
            <div>
                ...
                <div class="flex space-x-4">
                    <label class="flex-1 cursor-pointer">
                        <input type="radio" name="type" value="mcq" {{ old('type', $question->type) === 'mcq' ? 'checked' : '' }} class="sr-only peer" onchange="toggleOptions()">
                    ...
                    <label class="flex-1 cursor-pointer">
                        <input type="radio" name="type" value="descriptive" {{ old('type', $question->type) === 'descriptive' ? 'checked' : '' }} class="sr-only peer" onchange="toggleOptions()">
            ...
            <div>
                ...
                <textarea name="question" rows="3" required class="ios-input">{{ old('question', $question->question) }}</textarea>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Marks</label>
                <input type="number" name="marks" value="{{ old('marks', $question->marks) }}" required min="1" class="ios-input w-32">
            </div>
        </div>
                ...
                @foreach(['A', 'B', 'C', 'D'] as $key)
                    <div class="flex items-center space-x-3">
                        <label class="cursor-pointer">
                            <input type="radio" name="correct_answer" value="{{ $key }}" {{ old('correct_answer', $question->correct_answer) === $key ? 'checked' : '' }} class="sr-only peer">
                        ...
                        <input type="text" name="options[{{ $key }}]" value="{{ old("options.$key", $question->options[$key] ?? '') }}" class="ios-input flex-1" placeholder="Option {{ $key }}">
                    </div>
                @endforeach
            </div>
        ...
        <div class="ios-card p-6 space-y-6">
            <div>
                ...
                <textarea name="explanation" rows="2" class="ios-input">{{ old('explanation', $question->explanation) }}</textarea>
            </div>
        </div>
        ...
        <div class="flex items-center justify-end space-x-4">
            ...
            <button type="submit" class="ios-btn bg-ios-blue text-white hover:bg-blue-600">Update Question</button>
        </div>
    </form>
</div>
